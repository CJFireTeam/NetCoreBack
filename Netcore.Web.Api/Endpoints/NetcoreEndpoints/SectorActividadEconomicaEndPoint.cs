...
{
    public class SectorActividadEconomicaEndPoint : IEndpoint
    {
        public IEndpointRouteBuilder AddRoutes(IEndpointRouteBuilder endpoints)
        {
            endpoints.MapGet("/api/sectoractividadeconomica/{actividad}", [Authorize] async (HttpContext httpContext, Netcore.ActivoFijo.Model.Context context,int actividad) =>
            {

                SectorActividadEconomicaController controller = new SectorActividadEconomicaController(httpContext, context);

                return await controller.Get(actividad);
